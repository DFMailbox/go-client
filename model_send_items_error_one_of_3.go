/*
DFMailbox API

DFMailbox is a decentralized way to send messages to other DiamondFire plots.

API version: 0.6.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the SendItemsErrorOneOf3 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SendItemsErrorOneOf3{}

// SendItemsErrorOneOf3 struct for SendItemsErrorOneOf3
type SendItemsErrorOneOf3 struct {
	Type interface{} `json:"type"`
	Title interface{} `json:"title"`
	// HTTP status code
	Status int32 `json:"status"`
	Detail string `json:"detail"`
	// An id assigned by DiamondFire to identify a plot, this ID can be used in /plot <plot_id>
	Sender int32 `json:"sender"`
	// An id assigned by DiamondFire to identify a plot, this ID can be used in /plot <plot_id>
	Receiver *int32 `json:"receiver,omitempty"`
}

type _SendItemsErrorOneOf3 SendItemsErrorOneOf3

// NewSendItemsErrorOneOf3 instantiates a new SendItemsErrorOneOf3 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSendItemsErrorOneOf3(type_ interface{}, title interface{}, status int32, detail string, sender int32) *SendItemsErrorOneOf3 {
	this := SendItemsErrorOneOf3{}
	this.Type = type_
	this.Title = title
	this.Status = status
	this.Detail = detail
	this.Sender = sender
	return &this
}

// NewSendItemsErrorOneOf3WithDefaults instantiates a new SendItemsErrorOneOf3 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSendItemsErrorOneOf3WithDefaults() *SendItemsErrorOneOf3 {
	this := SendItemsErrorOneOf3{}
	return &this
}

// GetType returns the Type field value
// If the value is explicit nil, the zero value for interface{} will be returned
func (o *SendItemsErrorOneOf3) GetType() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SendItemsErrorOneOf3) GetTypeOk() (*interface{}, bool) {
	if o == nil || IsNil(o.Type) {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *SendItemsErrorOneOf3) SetType(v interface{}) {
	o.Type = v
}

// GetTitle returns the Title field value
// If the value is explicit nil, the zero value for interface{} will be returned
func (o *SendItemsErrorOneOf3) GetTitle() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}

	return o.Title
}

// GetTitleOk returns a tuple with the Title field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *SendItemsErrorOneOf3) GetTitleOk() (*interface{}, bool) {
	if o == nil || IsNil(o.Title) {
		return nil, false
	}
	return &o.Title, true
}

// SetTitle sets field value
func (o *SendItemsErrorOneOf3) SetTitle(v interface{}) {
	o.Title = v
}

// GetStatus returns the Status field value
func (o *SendItemsErrorOneOf3) GetStatus() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *SendItemsErrorOneOf3) GetStatusOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Status, true
}

// SetStatus sets field value
func (o *SendItemsErrorOneOf3) SetStatus(v int32) {
	o.Status = v
}

// GetDetail returns the Detail field value
func (o *SendItemsErrorOneOf3) GetDetail() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Detail
}

// GetDetailOk returns a tuple with the Detail field value
// and a boolean to check if the value has been set.
func (o *SendItemsErrorOneOf3) GetDetailOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Detail, true
}

// SetDetail sets field value
func (o *SendItemsErrorOneOf3) SetDetail(v string) {
	o.Detail = v
}

// GetSender returns the Sender field value
func (o *SendItemsErrorOneOf3) GetSender() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Sender
}

// GetSenderOk returns a tuple with the Sender field value
// and a boolean to check if the value has been set.
func (o *SendItemsErrorOneOf3) GetSenderOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Sender, true
}

// SetSender sets field value
func (o *SendItemsErrorOneOf3) SetSender(v int32) {
	o.Sender = v
}

// GetReceiver returns the Receiver field value if set, zero value otherwise.
func (o *SendItemsErrorOneOf3) GetReceiver() int32 {
	if o == nil || IsNil(o.Receiver) {
		var ret int32
		return ret
	}
	return *o.Receiver
}

// GetReceiverOk returns a tuple with the Receiver field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SendItemsErrorOneOf3) GetReceiverOk() (*int32, bool) {
	if o == nil || IsNil(o.Receiver) {
		return nil, false
	}
	return o.Receiver, true
}

// HasReceiver returns a boolean if a field has been set.
func (o *SendItemsErrorOneOf3) HasReceiver() bool {
	if o != nil && !IsNil(o.Receiver) {
		return true
	}

	return false
}

// SetReceiver gets a reference to the given int32 and assigns it to the Receiver field.
func (o *SendItemsErrorOneOf3) SetReceiver(v int32) {
	o.Receiver = &v
}

func (o SendItemsErrorOneOf3) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SendItemsErrorOneOf3) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Type != nil {
		toSerialize["type"] = o.Type
	}
	if o.Title != nil {
		toSerialize["title"] = o.Title
	}
	toSerialize["status"] = o.Status
	toSerialize["detail"] = o.Detail
	toSerialize["sender"] = o.Sender
	if !IsNil(o.Receiver) {
		toSerialize["receiver"] = o.Receiver
	}
	return toSerialize, nil
}

func (o *SendItemsErrorOneOf3) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"type",
		"title",
		"status",
		"detail",
		"sender",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varSendItemsErrorOneOf3 := _SendItemsErrorOneOf3{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varSendItemsErrorOneOf3)

	if err != nil {
		return err
	}

	*o = SendItemsErrorOneOf3(varSendItemsErrorOneOf3)

	return err
}

type NullableSendItemsErrorOneOf3 struct {
	value *SendItemsErrorOneOf3
	isSet bool
}

func (v NullableSendItemsErrorOneOf3) Get() *SendItemsErrorOneOf3 {
	return v.value
}

func (v *NullableSendItemsErrorOneOf3) Set(val *SendItemsErrorOneOf3) {
	v.value = val
	v.isSet = true
}

func (v NullableSendItemsErrorOneOf3) IsSet() bool {
	return v.isSet
}

func (v *NullableSendItemsErrorOneOf3) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSendItemsErrorOneOf3(val *SendItemsErrorOneOf3) *NullableSendItemsErrorOneOf3 {
	return &NullableSendItemsErrorOneOf3{value: val, isSet: true}
}

func (v NullableSendItemsErrorOneOf3) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSendItemsErrorOneOf3) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


