...
// AddressKeyPairAsLookupInstanceAddress200Response is a convenience function that returns AddressKeyPair wrapped in LookupInstanceAddress200Response
func AddressKeyPairAsLookupInstanceAddress200Response(v *AddressKeyPair) LookupInstanceAddress200Response {
...
func (dst *LookupInstanceAddress200Response) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into AddressKeyPair
	err = newStrictDecoder(data).Decode(&dst.AddressKeyPair)
	if err == nil {
		jsonAddressKeyPair, _ := json.Marshal(dst.AddressKeyPair)
		if string(jsonAddressKeyPair) == "{}" { // empty struct
			dst.AddressKeyPair = nil
		} else {
			if err = validator.Validate(dst.AddressKeyPair); err != nil {
				dst.AddressKeyPair = nil
			} else {
				match++
			}
...
func (src LookupInstanceAddress200Response) MarshalJSON() ([]byte, error) {
	if src.AddressKeyPair != nil {
		return json.Marshal(&src.AddressKeyPair)
	}
...
func (obj *LookupInstanceAddress200Response) GetActualInstance() (interface{}) {
	...
	if obj.AddressKeyPair != nil {
		return obj.AddressKeyPair
	}
